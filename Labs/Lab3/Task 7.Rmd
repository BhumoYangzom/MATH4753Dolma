...
inputPanel(
  ...
  selectInput("plotg", "Choose plot type",
      list(`type` = c("points alone", "linear regression line", "line segments")
      )
    ),
  
  textAreaInput("textg", "Title", value = "Yangzom Dolma", resize = "horizontal")
)
...
gpoints = g + geom_point(aes(col=Height))
gline = g+ geom_line()
grl = g+ geom_smooth(method = "lm")
gc = g + geom_density2d()
  
 if(input$plotg == "points alone")  {
   print(gpoints)
   ggsave("pointsalone.png", path = getwd())
 }
 
 if(input$plotg == "linear regression line")   {
   print(grl)
   ggsave("regression.png", path = getwd())
 }
